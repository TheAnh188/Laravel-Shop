    ...
    @can('accessibility', $authorization['canonical'])
        <div class="text-left">
            ...
            <div class="">
                ...
                @endif
                <form class="w-full mr-auto" method="POST" action="/product/{{ $product->id }}" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid md:grid-cols-2 md:gap-6">
                        ...
                        <div class="relative z-0 w-full mb-5 group">
                            <input type="text" name="code" id="code"
                                class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer"
                                placeholder="" value="{{ old('code', $product->code ?? 0) }}" />
                            <label for="code"
                    ...
                    <div class="grid md:grid-cols-2 md:gap-6">
                        <div class="relative z-0 w-full mb-5 group">
                            <input type="text" name="made_in" id="made_in"
                                class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer"
                                placeholder="" value="{{ old('made_in', $product->made_in) }}" />
                            <label for="made_in"
                    ...
                    <div class="relative w-full mb-5 group">
                        ...
                        <textarea id="description" rows="4" name="description"
                            ...
                            placeholder="{{ __('messages.description') }}">{{ old('description', $product->description) }}</textarea>
                    </div>
                    <div class="relative w-full mb-5 group">
                        <label for="content"
                        ...
                        <textarea id="content" rows="4" name="content"
                            class="editor block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                            placeholder="{{ __('messages.content') }}">{{ old('content', $product->description) }}</textarea>
                    </div>
                    <div class="relative z-0 w-full mb-5 group">
                        <label for="seo"
                        ...
                        <div class="bg-white border-2 rounded-lg p-4">
                            <div class="w-full text-[#1a0dab] meta_title-preview">
                                {{ old('meta_title', $product->meta_title) ?? __('messages.meta_title-preview') }}
                            </div>
                            <div class="w-full text-green-600 canonical-preview">
                                {{ env('APP_URL') . '/' . old('canonical', $product->canonical) ?? __('messages.canonical-preview') }}</div>
                            <div class="w-full text-gray-500 meta_description-preview">
                                {{ old('meta_description', $product->meta_description) ?? __('messages.meta_description-preview') }}</div>
                        </div>
                    </div>
                    <div class="relative z-0 w-full mb-5 group">
                    ...
                    <div class="relative z-0 w-full mb-5 group">
                        <textarea id="meta_description" rows="4" name="meta_description"
                            ...
                            placeholder="{{ __('messages.meta_description') }}">{{ old('meta_description', $product->meta_description) }}</textarea>
                    </div>
                    <div class="relative z-0 w-full mb-5 group">
                        <input type="text" name="canonical" id="canonical"
                            class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer"
                            placeholder="" value="{{ env('APP_URL') . '/' . old('canonical', $product->canonical) ?? env('APP_URL') }}" />
                        <label for="canonical"
                    ...
                    <div class="relative z-0 w-full mb-5 group">
                        ...
                        <img class="image-target cursor-pointer w-[157px] h-[148px] object-cover"
                            src="{{ $product->image ?? asset('images/no_image.jpg') }}"
                            alt="">
                        ...
                        <input name='image' type="text" value="{{ old('image', $product->image) }}" hidden>
                    </div>
                    <div class="relative z-0 w-full mb-5 group">
                    ...
                    <div class="w-full bg-white rounded-lg p-2 mb-5">
                        ...
                        <div class="flex justify-start items-center mt-4">
                            <input type="checkbox" value="1" id="variant-checkbox" class="w-5 h-5 setup-variant"
                                name="accept" {{ (old('accept') || count($product->product_variants) > 0) ? 'checked' : '' }}>
                        ...
                        @php
                            $attribute_catalogue = old('attributeCatalogue', (isset($product->attribute_catalogue) ? json_decode($product->attribute_catalogue, TRUE) : []));
                            // dd($attribute_catalogue);
                        @endphp
                        <div class="{{ count($attribute_catalogue) ? '' : 'hidden' }} variant-info">
                            <div class="variant-info-body">
                                @if ($attribute_catalogue)
                                    @foreach ($attribute_catalogue as $attrKey => $attrValue)
...
@section('script')
    <script>
        ...
        var attribute = @json(old('attribute', (isset($product->attributes) ? json_decode($product->attributes, TRUE) : [])));
        var variant = '{{ base64_encode(json_encode(old('variant', (isset($product->variants) ? json_decode($product->variants, TRUE) : [])))) }}';
    </script>
