    ...
    <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
        <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
            <tr>
                ...
                <th scope="col" class="w-[50%] px-4 py-3">{{ __('messages.post_catalogue.index.name') }}</th>
                ...
                @foreach ($post_catalogues as $post_catalogue)
                    <tr
                        ...
                        <td scope="row"
                            ...
                            <input type="checkbox" value="{{ $post_catalogue->id }}"
                        ...
                        @foreach ($languagess as $language)
                            ...
                            @endif
                            @php
                                $isTranslated =  $post_catalogue->languages->contains('id', $language->id);
                            @endphp
                            <td class="px-4 py-4 text-nowrap text-center">
                                <a href="/language/{{ $post_catalogue->id }}/{{ $language->id }}/PostCatalogue/translate" class="{{ $isTranslated ? 'text-blue-500' : 'text-red-500' }}">{{ $isTranslated ? 'Đã dịch' : 'Chưa dịch' }}</a>
                            </td>
                        @endforeach
                        <td class="px-4 py-4">
                            <label class="inline-flex items-center cursor-pointer">
                                <input data-modelId="{{ $post_catalogue->id }}" data-model="{{ $model }}"
                                    data-field="status" type="checkbox" class="sr-only peer status"
                                    value="{{ $post_catalogue->status }}"
                                    {{ $post_catalogue->status == 1 ? 'checked' : '' }}>
                                <div
                        ...
                        <td class="px-4 py-4">
                            <a href="/post-catalogue/{{ $post_catalogue->id }}/edit"
                                class="removeSelectedImage bg-[#1947ee] text-white px-3 py-2.5 rounded-md"><i
                                    class="fa-regular fa-pen-to-square"></i></a>
                            <a href="/post-catalogue/{{ $post_catalogue->id }}/delete"
...
{{ $post_catalogues->links('vendor.pagination.tailwind') }}
