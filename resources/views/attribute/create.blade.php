...
@section('content')
    @include('attribute.components.breadcrumb', [
        'title' => __('messages.attribute.create.title'),
        'title_table' => __('messages.attribute.create.title'),
    ])
    @can('accessibility', $authorization['canonical'])
        <div class="grid md:grid-cols-[40%,1fr] md:gap-6 mt-4 text-left">
            ...
            <div class="">
                ...
                @endif
                <form class="w-full mr-auto" method="POST" action="/attribute" enctype="multipart/form-data">
                    @csrf
                    <div class="grid md:grid-cols-2 md:gap-6">
                        <div class="relative z-0 w-full mb-5 group">
                            <select name="attribute_catalogue_id"
                                ...
                                @foreach ($dropdown as $key => $value)
                                    <option @if (old('attribute_catalogue_id') == $key) selected @endif value="{{ $key }}">
                                        {{ $value }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="relative z-0 w-full mb-5 group">
                            <select multiple name="catalogue[]"
                                class="select2 bg-white text-gray-900 text-sm rounded-lg block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white border-r-8 border-solid border-white">
                                @foreach ($dropdown as $key => $value)
                                    <option @if (is_array(old('catalogue', isset($attribute->catalogue) ? $attribute->catalogue : [])) &&
                                            in_array($key, old('catalogue', isset($attribute->catalogue) ? $attribute->catalogue : []))) selected @endif value="{{ $key }}">
                                        {{ $value }}</option>
                                @endforeach
                            </select>
                        </div>
                    ...
                    <div class="relative z-0 w-full mb-5 group">
                        ...
                        <label for="name"
                            class="peer-focus:font-medium absolute text-sm text-gray-500 dark:text-gray-400 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:start-0 rtl:peer-focus:translate-x-1/4 rtl:peer-focus:left-auto peer-focus:text-blue-600 peer-focus:dark:text-blue-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">{{ __('messages.attribute.index.name') }}
