<div>
    <form class="w-full grid md:grid-cols-5 md:gap-6 mt-2" action="/attribute">
        @php
        ...
        @php
            $attribute_catalogue_id = request('attribute_catalogue_id') ?: old('attribute_catalogue_id');
        @endphp
        <select id="countries" name="attribute_catalogue_id"
            ...
            @foreach ($dropdown as $key => $value)
                <option value="{{ $key }}" {{ $attribute_catalogue_id == $key ? 'selected' : '' }}>{{ $value }}</option>
            @endforeach
        </select>
        @php
