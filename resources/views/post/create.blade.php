...
@section('content')
    @include('post.components.breadcrumb', [
        'title' => __('messages.post.create.title'),
        'title_table' => __('messages.post.create.title'),
    ])
    @can('accessibility', $authorization['canonical'])
        <div class="grid md:grid-cols-[40%,1fr] md:gap-6 mt-4 text-left">
            ...
            <div class="">
                ...
                @endif
                <form class="w-full mr-auto" method="POST" action="/post" enctype="multipart/form-data">
                    @csrf
                        ...
                        <div class="relative z-0 w-full mb-5 group">
                            <select name="post_catalogue_id"
                                ...
                                @foreach ($dropdown as $key => $value)
                                    <option @if (old('post_catalogue_id') == $key) selected @endif value="{{ $key }}">
                                        {{ $value }}</option>
                                @endforeach
                            </select>
                        </div>
                    ...
                    <div class="relative z-0 w-full mb-5 group">
                        ...
                        <label for="name"
                            class="peer-focus:font-medium absolute text-sm text-gray-500 dark:text-gray-400 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:start-0 rtl:peer-focus:translate-x-1/4 rtl:peer-focus:left-auto peer-focus:text-blue-600 peer-focus:dark:text-blue-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">{{ __('messages.post.index.name') }}
