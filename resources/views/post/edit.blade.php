...
@section('content')
    @include('post.components.breadcrumb', [
        'title' => __('messages.post.edit.title'),
        'title_table' => __('messages.post.edit.title'),
    ])
    @can('accessibility', $authorization['canonical'])
        <div class="grid md:grid-cols-[40%,1fr] md:gap-6 mt-4 text-left">
            ...
            <div class="">
                ...
                @endif
                <form class="w-full mr-auto" method="POST" action="/post/{{ $post->id }}"
                    ...
                    <div class="grid md:grid-cols-2 md:gap-6">
                        <div class="relative z-0 w-full mb-5 group">
                            <select name="post_catalogue_id"
                                ...
                                @foreach ($dropdown as $key => $value)
                                    <option @if (old('post_catalogue_id', $post->post_catalogue_id ?? '') == $key) selected @endif value="{{ $key }}">
                                        {{ $value }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="relative z-0 w-full mb-5 group">
                            <select multiple name="catalogue[]"
                                class="select2 bg-white text-gray-900 text-sm rounded-lg block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white border-r-8 border-solid border-white">
                                @foreach ($dropdown as $key => $value)
                                    <option @if (is_array(old('catalogue', isset($catalogues) ? $catalogues : []))  && $key !== $post->post_catalogue_id &&
                        ...
                        <div class="relative z-0 w-full mb-5 group">
                            <select name="follow"
                                ...
                                @foreach (__('messages.follow_input') as $key => $value)
                                    <option @if (old('follow', $post->follow ?? '') == $key) selected @endif value="{{ $key }}">
                                        {{ $value }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="relative z-0 w-full mb-5 group">
                            <select name="status"
                                ...
                                @foreach (__('messages.status_input') as $key => $value)
                                    <option @if (old('status', $post->status ?? '') == $key) selected @endif value="{{ $key }}">
                                        {{ $value }}</option>
                                @endforeach
                            </select>
                        </div>
