<div>
    <form class="w-full grid md:grid-cols-5 md:gap-6 mt-2" action="/post">
        @php
        ...
        @endphp
        <select id="countries" name="post_catalogue_id"
            ...
            @foreach ($dropdown as $key => $value)
                <option value="{{ $key }}" {{ $post_catalogue_id == $key ? 'selected' : '' }}>{{ $value }}</option>
            @endforeach
        </select>
        @php
        ...
        <div class="relative">
            ...
            @include('layouts.admin.status_button', ['model' => 'Post'])
        </div>
    </form>
</div>
