...
@section('content')
    @include('attribute_catalogue.components.breadcrumb', [
        'title' => __('messages.attribute_catalogue.edit.title'),
        'title_table' => __('messages.attribute_catalogue.edit.title'),
    ])
    @can('accessibility', $authorization['canonical'])
        <div class="grid md:grid-cols-[40%,1fr] md:gap-6 mt-4 text-left">
            ...
            <div class="">
                ...
                @endif
                <form class="w-full mr-auto" method="POST" action="/attribute-catalogue/{{ $attribute_catalogue->id }}"
                        ...
                        <div class="relative z-0 w-full mb-5 group">
                            <select name="parent_id"
                                ...
                                @foreach ($dropdown as $key => $value)
                                    <option @if (old('parent_id', $attribute_catalogue->parent_id ?? '') == $key) selected @endif value="{{ $key }}">
                                        {{ $value }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="relative z-0 w-full mb-5 group">
                            <select name="status"
                                ...
                                @foreach (__('messages.status_input') as $key => $value)
                                    <option @if (old('status', $attribute_catalogue->status ?? '') == $key) selected @endif value="{{ $key }}">
                                        {{ $value }}</option>
                                @endforeach
                            </select>
                        </div>
                    ...
                    <div class="relative z-0 w-full mb-5 group">
                        <select name="follow"
                            ...
                            @foreach (__('messages.follow_input') as $key => $value)
                                <option @if (old('follow', $attribute_catalogue->follow ?? '') == $key) selected @endif value="{{ $key }}">
                                    {{ $value }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="relative z-0 w-full mb-5 group">
                        <input type="text" name="name" id="name"
                            ...
                            placeholder="" value="{{ old('name', $attribute_catalogue->name ?? '') }}" />
                        <label for="name"
                    ...
                    <div class="relative w-full mb-5 group">
                        ...
                        <textarea id="description" rows="4" name="description"
                            ...
                            placeholder="{{ __('messages.description') }}">{{ old('description', $attribute_catalogue->description ?? '') }}</textarea>
                    </div>
                    <div class="relative w-full mb-5 group">
                        <label for="content"
                        ...
                        <textarea id="content" rows="4" name="content"
                            class="editor block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                            placeholder="{{ __('messages.content') }}">{{ old('content', $attribute_catalogue->content ?? '') }}</textarea>
                    </div>
                    <div class="relative z-0 w-full mb-5 group">
                        <label for="seo"
                        ...
                        <div class="bg-white border-2 rounded-lg p-4">
                            <div class="w-full text-[#1a0dab] meta_title-preview">
                                {{ old('meta_title', $attribute_catalogue->meta_title ?? '') ?? __('messages.meta_title-preview') }}
                            </div>
                            <div class="w-full text-green-600 canonical-preview">
                                {{ env('APP_URL') . '/' . old('canonical', $attribute_catalogue->canonical ?? '') ?? __('messages.canonical-preview') }}
                            </div>
                            <div class="w-full text-gray-500 meta_description-preview">
                                {{ old('meta_description', $attribute_catalogue->meta_description ?? '') ?? __('messages.meta_description-preview') }}
                            </div>
                        </div>
                    </div>
                    <div class="relative z-0 w-full mb-5 group">
                        <input type="text" name="meta_title" id="meta_title"
                            class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer"
                            placeholder="" value="{{ old('meta_title', $attribute_catalogue->meta_title ?? '') }}" />
                    ...
                    <div class="relative z-0 w-full mb-5 group">
                        <input type="text" name="meta_keyword" id="meta_keyword"
                            class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer"
                            placeholder="" value="{{ old('meta_keyword', $attribute_catalogue->meta_keyword ?? '') }}" />
                    ...
                    <div class="relative z-0 w-full mb-5 group">
                        <textarea id="meta_description" rows="4" name="meta_description"
                            ...
                            placeholder="{{ __('messages.meta_description') }}">{{ old('meta_description', $attribute_catalogue->meta_description ?? '') }}</textarea>
                    </div>
                    <div class="relative z-0 w-full mb-5 group">
                        <input type="text" name="canonical" id="canonical"
                            class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer"
                            placeholder=""
                            value="{{ env('APP_URL') . '/' . old('canonical', $attribute_catalogue->canonical ?? '') ?? env('APP_URL') . '/' }}" />
                        <label for="canonical"
                    ...
                    <div class="relative z-0 w-full mb-5 group">
                        ...
                        <img class="image-target cursor-pointer w-[157px] h-[148px] object-cover"
                            src="{{ empty($attribute_catalogue->image) ? asset('images/no_image.jpg') : old('image', $attribute_catalogue->image, asset('images/no_image.jpg')) }}"
                            alt="">
                        ...
                        <input name='image' type="text" value="{{ old('image', $attribute_catalogue->image ?? '') }}"
                            hidden>
                    </div>
                    <div class="relative z-0 w-full mb-5 group">
