    ...
    <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
        <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
            <tr>
                ...
                <th scope="col" class="w-[50%] px-4 py-3">{{ __('messages.attribute_catalogue.index.name') }}</th>
        ...
        <tbody>
            @if (isset($attribute_catalogues) && is_object($attribute_catalogues))
                @foreach ($attribute_catalogues as $attribute_catalogue)
                    <tr
                        ...
                        <td scope="row"
                            ...
                            <input type="checkbox" value="{{ $attribute_catalogue->id }}"
                                class="input-checkbox checkbox-item">
                        </td>
                        <td class="px-4 py-4">
                            <span>{{ str_repeat('|----', $attribute_catalogue->level > 0 ? $attribute_catalogue->level - 1 : 0) . $attribute_catalogue->name }}</span>
                        </td>
                        @foreach ($languagess as $language)
                            @if (session('app_locale') === $language->canonical)
                                @continue
                            @endif
                            @php
                                $isTranslated =  $attribute_catalogue->languages->contains('id', $language->id);
                            @endphp
                            <td class="px-4 py-4 text-nowrap text-center">
                                <a href="/language/{{ $attribute_catalogue->id }}/{{ $language->id }}/AttributeCatalogue/translate" class="{{ $isTranslated ? 'text-blue-500' : 'text-red-500' }}">{{ $isTranslated ? 'Đã dịch' : 'Chưa dịch' }}</a>
                            </td>
                        @endforeach
                        <td class="px-4 py-4">
                            <label class="inline-flex items-center cursor-pointer">
                                <input data-modelId="{{ $attribute_catalogue->id }}" data-model="{{ $model }}"
                                    data-field="status" type="checkbox" class="sr-only peer status"
                                    value="{{ $attribute_catalogue->status }}"
                                    {{ $attribute_catalogue->status == 1 ? 'checked' : '' }}>
                                <div
                        ...
                        <td class="px-4 py-4">
                            <a href="/attribute-catalogue/{{ $attribute_catalogue->id }}/edit"
                                class="removeSelectedImage bg-[#1947ee] text-white px-3 py-2.5 rounded-md"><i
                                    class="fa-regular fa-pen-to-square"></i></a>
                            <a href="/attribute-catalogue/{{ $attribute_catalogue->id }}/delete"
...
{{ $attribute_catalogues->links('vendor.pagination.tailwind') }}
