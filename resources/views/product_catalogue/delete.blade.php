...
@section('content')
    @include('product_catalogue.components.breadcrumb', [
        'title' => __('messages.product_catalogue.delete.title'),
        'title_table' => __('messages.product_catalogue.delete.title'),
    ])
        ...
        <div class="grid md:grid-cols-[40%,1fr] md:gap-6 mt-4 text-left">
            <div class="">
                <p class="text-gray-600">{{ __('messages.notice.delete') }} <span
                        class="text-red-400">{{ $product_catalogue->name }}</span></p>
            ...
            <div class="">
                ...
                @endif
                <form class="max-w-md mr-auto" method="POST" action="/product-catalogue/{{ $product_catalogue->id }}">
                    @csrf
                    @method('DELETE')
                    <div class="relative z-0 w-full mb-5 group">
                        <input type="text" name="name" id="name"
                            class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer"
                            placeholder="" value="{{ old('name', $product_catalogue->name ?? '') }}" readonly />
                        <label for="name"
