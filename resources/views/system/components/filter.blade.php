<div>
    <form class="w-full grid md:grid-cols-5 md:gap-6 mt-2" action="/product">
        @php
        ...
        @php
            $product_catalogue_id = request('product_catalogue_id') ?: old('product_catalogue_id');
        @endphp
        <select id="countries" name="product_catalogue_id"
            ...
            @foreach ($dropdown as $key => $value)
                <option value="{{ $key }}" {{ $product_catalogue_id == $key ? 'selected' : '' }}>{{ $value }}</option>
            @endforeach
        </select>
        @php
        ...
        <div class="relative">
            ...
            @include('layouts.admin.status_button', ['model' => 'Product'])
        </div>
    </form>
</div>
