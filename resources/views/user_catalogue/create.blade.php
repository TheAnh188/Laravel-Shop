...
@section('content')
    @include('user_catalogue.components.breadcrumb', [
        'title' => __('messages.user_catalogue.create.title'),
        'title_table' => __('messages.user_catalogue.create.title'),
    ])
    @can('accessibility', $authorization['canonical'])
        <div class="grid md:grid-cols-[40%,1fr] md:gap-6 mt-4 text-left">
            ...
            <div class="">
                ...
                @endif
                <form class="max-w-md " method="POST" action="/user-catalogue">
                    @csrf
                    ...
                    <div class="relative z-0 w-full mb-5 group">
                        <label for="description" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">{{ __('messages.user_catalogue.index.description') }}</label>
                        <textarea id="description" rows="4" aria-valuetext="{{ old('description') }}" name="description"
                            ...
                            placeholder="{{ __('messages.user_catalogue.index.description') }}"></textarea>
                    </div>
                    <button type="submit"
