...
@section('content')
    @include('{tableName}.components.breadcrumb', [
        'title' => __('messages.{tableName}.create.title'),
        'title_table' => __('messages.{tableName}.create.title'),
    ])
    @can('accessibility', $authorization['canonical'])
        <div class="grid md:grid-cols-[40%,1fr] md:gap-6 mt-4 text-left">
            ...
            <div class="">
                ...
                @endif
                <form class="w-full mr-auto" method="POST" action="/{moduleCanonical}" enctype="multipart/form-data">
                    @csrf
                    <div class="grid md:grid-cols-2 md:gap-6">
                        <div class="relative z-0 w-full mb-5 group">
                            <select name="{tableName}_catalogue_id"
                                ...
                                @foreach ($dropdown as $key => $value)
                                    <option @if (old('{tableName}_catalogue_id') == $key) selected @endif value="{{ $key }}">
                                        {{ $value }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="relative z-0 w-full mb-5 group">
                            <select multiple name="catalogue[]"
                                class="select2 bg-white text-gray-900 text-sm rounded-lg block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white border-r-8 border-solid border-white">
                                @foreach ($dropdown as $key => $value)
                                    <option @if (is_array(old('catalogue', isset(${tableName}->catalogue) ? ${tableName}->catalogue : [])) &&
                                            in_array($key, old('catalogue', isset(${tableName}->catalogue) ? ${tableName}->catalogue : []))) selected @endif value="{{ $key }}">
                                        {{ $value }}</option>
                                @endforeach
                            </select>
                        </div>
                    ...
                    <div class="relative z-0 w-full mb-5 group">
                        ...
                        <div class="bg-white border-2 rounded-lg p-4">
                            <div class="w-full text-[#1a0dab] meta_title-preview">{{ old('meta_title') ?? __('messages.meta_title-preview') }}
                            </div>
                    ...
                    <div class="relative z-0 w-full mb-5 group">
                        <label class="block mb-1 text-sm font-medium text-gray-900 dark:text-white" for="user_avatar">{{ __("messages.image") }}</label>
