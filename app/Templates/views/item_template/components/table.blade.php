    ...
    <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
        <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
            <tr>
                ...
                <th scope="col" class="w-[51%] px-4 py-3">{{ __('messages.{tableName}.index.name') }}</th>
                ...
                @foreach (${tableName}s as ${tableName})
                    <tr id="{{ ${tableName}->id }}"
                        ...
                        <td scope="row"
                            ...
                            <input type="checkbox" value="{{ ${tableName}->id }}" class="input-checkbox checkbox-item">
                        </td>
                        <td class="px-4 py-4">
                            <div class="flex items-center">
                                <img src="{{ ${tableName}->image ?? asset('images/no_image.jpg') }}" alt="Image" class="w-[10%] object-cover mr-2">
                            ...
                            <div>
                                ...
                                @foreach (${tableName}->{tableName}_catalogues as ${tableName}_catalogue)
                                    @foreach (${tableName}_catalogue->{tableName}_catalogue_language as $value)
                                        <a href="/{moduleCanonical}-catalogue" class="text-[#333] font-sf-ui font-light hover:underline hover:text-blue-400">{{ $value->name }}</a>
                                    @endforeach
                                @endforeach
                            </div>
                        </td>
                        ...
                        @endforeach
                        <td class="px-4 py-4">
                            <input type="text" value="{{ ${tableName}->order }}" name="order"
