                ...
                <form class="w-full mr-auto" method="POST" action="/{moduleCanonical}/{{ ${tableName}->id }}"
                    ...
                    <div class="grid md:grid-cols-2 md:gap-6">
                        <div class="relative z-0 w-full mb-5 group">
                            <select name="{tableName}_catalogue_id"
                                ...
                                @foreach ($dropdown as $key => $value)
                                    <option @if (old('{tableName}_catalogue_id', ${tableName}->{tableName}_catalogue_id ?? '') == $key) selected @endif value="{{ $key }}">
                                        {{ $value }}</option>
                                @endforeach
                            </select>
                        </div>
                        @php
                            $catalogues = [];
                            if(isset(${tableName})) {
                                foreach (${tableName}->{tableName}_catalogues as $key => $value) {
                                    $catalogues[] = $value->id;
                                }
                            }
                        @endphp
                        <div class="relative z-0 w-full mb-5 group">
                            <select multiple name="catalogue[]"
                                class="select2 bg-white text-gray-900 text-sm rounded-lg block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white border-r-8 border-solid border-white">
                                @foreach ($dropdown as $key => $value)
                                    <option @if (is_array(old('catalogue', isset($catalogues) ? $catalogues : []))  && $key !== ${tableName}->{tableName}_catalogue_id &&
                    ...
                    <div class="relative w-full mb-5 group">
                        ...
                        <textarea id="description" rows="4" name="description"
                            ...
                            placeholder="Mô tả">{{ old('description', ${tableName}->description ?? '') }}</textarea>
                    </div>
                    <div class="relative w-full mb-5 group">
                        ...
                        <textarea id="content" rows="4" name="content"
                            class="editor block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                            placeholder="Mô tả nội dung">{{ old('content', ${tableName}->content ?? '') }}</textarea>
                    </div>
                    <div class="relative z-0 w-full mb-5 group">
                        ...
                        <div class="bg-white border-2 rounded-lg p-4">
                            <div class="w-full text-[#1a0dab] meta_title-preview">
                                {{ old('meta_title', ${tableName}->meta_title ?? '') ?? 'Tiêu đề SEO' }}</div>
                            <div class="w-full text-green-600 canonical-preview">
                                {{ env('APP_URL') . '/' . old('canonical', ${tableName}->canonical ?? '') ?? 'https://duong-dan-cua-ban.html' }}
                            </div>
                            <div class="w-full text-gray-500 meta_description-preview">
                                {{ old('meta_description', ${tableName}->meta_description ?? '') ?? 'Mô tả trang web ...' }}
                            </div>
                        </div>
                    </div>
                    <div class="relative z-0 w-full mb-5 group">
                    ...
                    <div class="relative z-0 w-full mb-5 group">
                        <textarea id="meta_description" rows="4" name="meta_description"
                            ...
                            placeholder="Mô tả SEO">{{ old('meta_description', ${tableName}->meta_description ?? '') }}</textarea>
                    </div>
                    <div class="relative z-0 w-full mb-5 group">
                    ...
                    <div class="relative z-0 w-full mb-5 group">
                        ...
                        <img class="image-target cursor-pointer w-[157px] h-[148px] object-cover"
                            src="{{ ${tableName}->image ?? asset('images/no_image.jpg') }}" alt="">
